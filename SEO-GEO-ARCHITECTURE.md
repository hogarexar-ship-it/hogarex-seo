...
- **`/{rubro}/caba/`:** desde 2026-08-28, por decisión explícita del cliente, usa el mismo patrón "header-only" que las páginas de barrio: el body es idéntico al hub del rubro (`/{rubro}/`), solo cambia el header (title, meta description, canonical, og:*, JSON-LD, breadcrumb, H1, hero-sub) y el segundo botón del hero ("Ver el hub de {rubro}" en vez de "Buscar Profesionales"). Ya no tiene contenido propio reducido — se prioriza consistencia visual y de contenido sobre la diferenciación de ángulo. Si en el futuro Google penaliza esto por contenido duplicado, revisar esta decisión.
